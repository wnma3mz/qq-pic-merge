...
class Merge(object):
    ...
    def download(self, snum):
        url = "https://qlogo4.store.qq.com/qzone/{}/{}/50".format(snum, snum)
        img = requests.get(url).content
        return img

    def get_picarr(self, fpname=None, picbytes=None):
    ...
    def convert_pic(self, fpname, img):
    ...
    def get_array(self):
        ...
        for num in self.uin_lst:
            snum = str(num)
            # 获取图片字节码
            img = self.download(snum)
            mat = None
            if (img == QQZONE_ONE_BYTES) or (img == QQZONE_TWO_BYTES):
                continue
            try:
                # 字节流可以直接读取
                mat = self.get_picarr(picbytes=img)
                self.pic_mat.append(mat)
            except OSError:
                # 不能被直接读取，需要下载下来进行格式转换
                self.convert_pic(snum, img)
                mat = self.get_picarr(fpname=num)
                self.pic_mat.append(mat)
